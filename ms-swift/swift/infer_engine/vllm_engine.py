...
def _patch_qwen25vl_mrope_config(config):
    """Avoid vLLM rejecting HF's Qwen2.5-VL mrope compatibility fields."""
    for cfg in (config, getattr(config, 'text_config', None)):
        if cfg is None:
            continue
        for attr in ('rope_scaling', 'rope_parameters'):
            rope_config = getattr(cfg, attr, None)
            if not isinstance(rope_config, dict):
                continue
            if rope_config.get('type') == 'mrope' and rope_config.get('rope_type') == 'default':
                rope_config.pop('type', None)
    ...
    def _patch_auto_config(self):
        ...
        def _from_pretrained(*args, **kwargs):
            ...
            if self.model_type == 'qwen2_5_vl':
                _patch_qwen25vl_mrope_config(config)
