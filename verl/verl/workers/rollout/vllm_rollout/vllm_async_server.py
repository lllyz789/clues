...
def _patch_qwen2_tokenizer_for_vllm() -> None:
    """Patch Transformers 5 Qwen2Tokenizer for vLLM's tokenizer cache."""
    try:
...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        if skip_tokenizer_init and self._is_multimodal_model():
            logger.warning(
                "Disabling skip_tokenizer_init for multimodal model %s because vLLM needs tokenizer-backed "
    ...
    async def run_server(self, args: argparse.Namespace):
        _patch_qwen2_tokenizer_for_vllm()
    ...
    def _is_multimodal_model(self) -> bool:
        """Return whether the HF config describes a model with non-text inputs."""
        hf_config = self.model_config.hf_config
        if any(hasattr(hf_config, attr) for attr in ("vision_config", "audio_config", "video_config")):
            return True

        architectures = self.model_config.architectures or []
        return any(any(marker in arch for marker in ("VL", "Omni", "Vision", "Audio")) for arch in architectures)
