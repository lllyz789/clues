    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Per-component reward extra info metrics
                if reward_extra_infos_dict:
                    for key, values in reward_extra_infos_dict.items():
                        if key == "score":
                            continue
                        vals = np.asarray(values, dtype=np.float32)
                        metrics[f"reward_components/{key}/mean"] = float(np.mean(vals))
                        metrics[f"reward_components/{key}/max"] = float(np.max(vals))
                        metrics[f"reward_components/{key}/min"] = float(np.min(vals))
