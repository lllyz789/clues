...
DISTILLATION_TOPK=${DISTILLATION_TOPK:-200}
DISTILLATION_LOSS_COEF=${DISTILLATION_LOSS_COEF:-0.05}
DISTILLATION_LOSS_COEF_LINEAR_DECAY=${DISTILLATION_LOSS_COEF_LINEAR_DECAY:-True}
DISTILLATION_LOSS_COEF_DECAY_STEPS=${DISTILLATION_LOSS_COEF_DECAY_STEPS:-50}
DISTILLATION_LOSS_COEF_END=${DISTILLATION_LOSS_COEF_END:-0.0}
...
"${PYTHON_BIN}" -m verl.trainer.main_ppo_sync \
    ...
    distillation.distillation_loss.distillation_loss_coef_linear_decay="${DISTILLATION_LOSS_COEF_LINEAR_DECAY}" \
    distillation.distillation_loss.distillation_loss_coef_decay_steps="${DISTILLATION_LOSS_COEF_DECAY_STEPS}" \
    distillation.distillation_loss.distillation_loss_coef_end="${DISTILLATION_LOSS_COEF_END}" \
