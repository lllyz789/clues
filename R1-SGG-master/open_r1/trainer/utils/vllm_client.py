...
class VLLMClient:
    ...
    def __init__(
        ...
        disable_weight_update: bool = False
    ):
        ...
        self.disable_weight_update = disable_weight_update
        if not disable_weight_update:
            self.init_communicator()
            atexit.register(self.close_communicator)  # when the client object is deleted, close the weight update group
        else:
            print("disable_weight_update=True!")
    ...
    def compute_log_probs(
        ...
        url = f"http://{self.host}:{self.server_port}/compute_log_probs/"
        payload = {
        ...
        if response.status_code == 200:
            return response.json()["per_token_logps"]
        else:
            raise Exception(f"Request failed: {response.status_code}, {response.text}")
